...
public class DruidTemplateParser extends AbstractTemplateParser {
    ...
    public <T extends Template> T convertTemplate(AppShadowDatabaseDTO dto) {
        ...
        DruidTemplate druidTemplate = JSONObject.parseObject(attachment, DruidTemplate.class);
        return (T) druidTemplate;
    }
    ...
    public ShadowDetailResponse convertDetailByTemplate(Long recordId) {
        ...
        DruidTemplate druidTemplate = Convert.convert(DruidTemplate.class, shaDowFileExtedn);
        ...
        if(DsTypeEnum.SHADOW_TABLE.getCode().equals(convert.getDsType())){
            ...
            List<String> protectArray = this.getProtectArray(DruidTemplate.class);
            ...
            for (String s : protectArray) {
                ...
                info.setKey(s);
                try {
                    field = druidTemplate.getClass().getDeclaredField(s);
                    field.setAccessible(true);
                    info.setValue((String) field.get(druidTemplate));
                } catch (Exception e) {
                    log.error("");//todo nf
                }
                extInfo.add(info);
