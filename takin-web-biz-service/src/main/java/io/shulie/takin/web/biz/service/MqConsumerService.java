...
import com.pamirs.takin.entity.dao.assist.mqconsumer.TMqMsgDao;
import com.pamirs.takin.entity.domain.entity.TMqMsg;
import io.shulie.takin.web.biz.common.CommonService;
import io.shulie.takin.web.biz.utils.GlobalVariableUtil;
...
public class MqConsumerService extends CommonService {
    ...
    public List<String> getPIDs(Map<String, Object> paraMap) {
        ...
        LOGGER.info("consume进程号集合为{}" + ProcessUtils.getPidList(sb.toString()));
     ...
     * @return
     */
    public int addMqMsg(TMqMsg tMqMsg) throws TakinModuleException {

        int mqMsgExist = TMqMsgDao.mqMsgExist(tMqMsg);
        if (mqMsgExist > 0) {

            throw new TakinModuleException(TakinErrorEnum.ASSIST_SAVE_MQ_DUPICATE);
        }
        String msgType = tMqMsg.getMsgType();
        if (MQEnum.IBM.toString().equals(msgType) && StringUtils.contains(tMqMsg.getEsbcode(), "|")) {

            throw new TakinModuleException(TakinErrorEnum.ASSIST_SAVE_MQ_PARAMERROR);
        }

        if (MQEnum.ESB.toString().equals(msgType) || MQEnum.IBM.toString().equals(msgType)) {
            tMqMsg.setTransportType("1");
        }
        Map<String, Object> mqMsgMap = GlobalVariableUtil.getValue(TakinDictTypeEnum.MQMSG);
        //先从内存中取数据，如果没有取到或取到空的，则从数据库中拿，然后放到内存中
        if (mqMsgMap == null || mqMsgMap.isEmpty()) {
            mqMsgMap = queryDicList(TakinDictTypeEnum.MQMSG);
            if (mqMsgMap == null) {
                throw new TakinModuleException(TakinErrorEnum.ASSIST_NOT_FOUND_MQTYPE_EXCEPTION);
            }
            GlobalVariableUtil.setValue(TakinDictTypeEnum.MQMSG, mqMsgMap);
        }
     ...
     *
     * @param tMqMsg mq消息
     * @throws TakinModuleException 找不到消息抛出该异常
     */
    public void updateMqMsg(TMqMsg tMqMsg) throws TakinModuleException {
        TMqMsg mqMsg = TMqMsgDao.selectMqMsgById(tMqMsg.getMsgId());
        if (mqMsg == null) {

            throw new TakinModuleException(TakinErrorEnum.ASSIST_MQMSG_NOT_FOUNT_EXCEPTION);
        }
    ...
    private Map<String, Object> getDictMap(String dictContent, TakinDictTypeEnum dictTypeEnum) {
    ...
    public PageInfo<TMqMsg> queryMqMsgList(Map<String, Object> paramMap) {
        ...
        if (MQConstant.ROCKETMQ.equals(msgType)) {
            List<TMqMsg> mqMsgList = TMqMsgDao.selectakincketMqMsgList(paramMap);
