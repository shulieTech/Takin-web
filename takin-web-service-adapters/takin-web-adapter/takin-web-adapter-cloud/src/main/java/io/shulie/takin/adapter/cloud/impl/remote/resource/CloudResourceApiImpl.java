...
public class CloudResourceApiImpl implements CloudResourceApi {
    ...
    public List<CloudResource> getDetails(int taskId, String resourceId) {
        ...
        crr.setPressureId(taskId);
        crr.setResourceId(resourceId);
        List<CloudResource> resources = cloudApiSenderService.get(EntrypointUrl.join(EntrypointUrl.MODULE_RRESSURE,EntrypointUrl.METHOD_RESOURCE_MACHINE), crr, new TypeReference<ApiResult<List<CloudResource>>>() {
        }).getData();
    ...
    @Override
    public Map<String, Integer> check(ResourceCheckRequest request) {
        return cloudApiSenderService.post(
                EntrypointUrl.join(EntrypointUrl.MODULE_RRESSURE, EntrypointUrl.MODULE_RESOURCE_CHECK),
                request, new TypeReference<ApiResult<Map<String, Integer>>>() {
                }).getData();
    }
    ...
    public String lock(ResourceLockRequest request) {
        return cloudApiSenderService.post(
                EntrypointUrl.join(EntrypointUrl.MODULE_RRESSURE, EntrypointUrl.METHOD_RESOURCE_LOCK),
    ...
    public void unLock(ResourceUnLockRequest request) {
        try {
            cloudApiSenderService.get(
                    EntrypointUrl.join(EntrypointUrl.MODULE_RRESSURE, EntrypointUrl.METHOD_RESOURCE_UNLOCK),
