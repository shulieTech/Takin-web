...
public interface AppDataSourceMapper
    ...
    @Insert("<script>" +
            ...
            "middleware_type=values(middleware_type),connection_pool=values(connection_pool),ext_info=values(ext_info),attachment=values(attachment),gmt_modified=now()" +
            "</script>")
    void saveOrUpdate(@Param("list") List<PressureResourceAppDataSourceEntity> list);
}
