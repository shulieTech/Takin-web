 ...
 */
@Component
public class ShadowHbaseConfigAgentCache extends AbstractAgentConfigCache<List<DsServerVO>> {

    public static final String CACHE_NAME = "t:a:c:shadow:hbase";

    @Autowired
    private DsService dsService;

    public ShadowHbaseConfigAgentCache(@Autowired RedisTemplate redisTemplate) {
        super(CACHE_NAME, redisTemplate);
    }

    @Override
    protected List<DsServerVO> queryValue(String namespace) {
        return dsService.getShadowDsServerConfigs(namespace, DsTypeEnum.SHADOW_HBASE_SERVER);
    }
