...
import io.shulie.takin.web.data.convert.performance.TraceManageResultConvertor;
import io.shulie.takin.web.data.mapper.mysql.TraceManageDeployMapper;
import io.shulie.takin.web.data.mapper.mysql.TraceManageMapper;
...
public class TraceManageDAOImpl implements TraceManageDAO{
    ...
    @Override
    public List<TraceManageDeployResult> queryTraceManageDeployByStatus(Integer status, TenantCommonExt ext) {
        LambdaQueryWrapper<TraceManageDeployEntity> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(TraceManageDeployEntity::getStatus,status);
        if (ext != null) {
            wrapper.eq(TraceManageDeployEntity::getTenantId,ext.getTenantId());
            wrapper.eq(TraceManageDeployEntity::getEnvCode,ext.getEnvCode());
        }
        List<TraceManageDeployEntity> traceManageDeployEntities = traceManageDeployMapper.selectList(wrapper);
        if (CollectionUtils.isNotEmpty(traceManageDeployEntities)){
            return traceManageDeployEntities.stream().map(this::getTraceManageDeployResult).collect(Collectors.toList());
