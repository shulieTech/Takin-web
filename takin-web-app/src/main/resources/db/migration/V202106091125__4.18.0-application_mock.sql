...
WHERE table_schema = DATABASE() AND TABLE_NAME = 't_business_link_manage_table' AND COLUMN_NAME = 'TYPE');

SET count2 = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS

WHERE table_schema = DATABASE() AND TABLE_NAME = 't_business_link_manage_table' AND COLUMN_NAME = 'BIND_BUSINESS_ID');
