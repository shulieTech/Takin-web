...
public class C3p0TemplateParser extends AbstractTemplateParser {
    ...
    public <T extends Template> T convertTemplate(AppShadowDatabaseDTO dto) {
        ...
        C3p0Template c3p0Template = JSONObject.parseObject(attachment, C3p0Template.class);
        return (T) c3p0Template;
    }
    ...
    public ShadowDetailResponse convertDetailByTemplate(Long recordId) {
        ...
        C3p0Template c3p0Template = Convert.convert(C3p0Template.class, shaDowFileExtedn);
        ...
        if(DsTypeEnum.SHADOW_TABLE.getCode().equals(convert.getDsType())){
            ...
            List<String> protectArray = this.getProtectArray(C3p0Template.class);
            ...
            for (String s : protectArray) {
                ...
                info.setKey(s);
                try {
                    field = c3p0Template.getClass().getDeclaredField(s);
                    field.setAccessible(true);
                    info.setValue((String) field.get(c3p0Template));
                } catch (Exception e) {
                    log.error("");//todo nf
                }
                extInfo.add(info);
