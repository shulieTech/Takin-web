...
CREATE TABLE aiops_shard_m.trace_e2e_assert_metrics (
  ...
  `createDate` DateTime DEFAULT toDateTime (
  now()) 
) ENGINE = ReplicatedMergeTree ( '/clickhouse/tables/aiops/trace_e2e_assert_metrics/{m_shard}', '{replica}' ) PARTITION BY traceId PRIMARY KEY ( traceId, nodeId, exceptionType ) 
