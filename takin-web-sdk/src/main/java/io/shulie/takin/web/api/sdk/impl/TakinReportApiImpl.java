...
public class TakinReportApiImpl implements TakinReportApi {
    @Override
    public ResponseResult queryStatus(AbstractTakinWebClient webClient, Long reportId) {
        ...
        result.setData(statusModel.getTaskStatus());

        return result;
    }

    @Override
    public ResponseResult queryReportSummary(AbstractTakinWebClient webClient, Long reportId) {
        final Map<String, Object> dataMap = new HashMap<>();
        dataMap.put("reportId", reportId);
        final String withBody = webClient.executeWithBody(
