...
CREATE TABLE aiops_shard_r.t_app_base_data (
  ...
  `createDate` DateTime DEFAULT toDateTime (
  now()) 
) ENGINE = ReplicatedMergeTree ( '/clickhouse/tables/cluster-aiops/t_app_base_data/{r_shard}', '{replica}' ) PARTITION BY app_name PRIMARY KEY ( agent_id, app_name, tenant_app_key, user_id ) 
...
CREATE TABLE aiops_shard_r.t_performance_base_data (
  ...
  `createDate` DateTime DEFAULT toDateTime (
  now()) 
) ENGINE = ReplicatedMergeTree ( '/clickhouse/tables/cluster-aiops/t_performance_base_data/{r_shard}', '{replica}' ) PARTITION BY app_name PRIMARY KEY ( app_name ) 
...
CREATE TABLE aiops_shard_r.trace_metrics (
  ...
  `createDate` DateTime DEFAULT toDateTime (
  now()) 
) ENGINE = ReplicatedMergeTree ( '/clickhouse/tables/cluster-aiops/trace_metrics/{r_shard}', '{replica}' ) PARTITION BY traceId PRIMARY KEY ( traceId, time, appName ) 
