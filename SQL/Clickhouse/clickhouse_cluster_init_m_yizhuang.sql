...
CREATE TABLE stresstest_shard_m.t_engine_metrics_all (
  `time` Int64,
  `transaction` String,
  `test_name` String,
  `count` Int16,
...
CREATE TABLE stresstest_shard_m.t_engine_pressure_all (
  `time` Int64,
  `transaction` String,
  `avg_rt` Decimal32 ( 4 ),
  `avg_tps` Decimal32 ( 4 ),
...
CREATE TABLE stresstest_shard_m.t_app_base_data (
  ...
  `createDate` DateTime DEFAULT toDateTime (
  now()) 
) ENGINE = ReplicatedMergeTree ( '/clickhouse/tables/aiops/t_app_base_data/{m_shard}', '{replica}' )  PARTITION BY modulo(time,3600000) PRIMARY KEY ( time,agent_id, app_name, tenant_app_key, user_id )
ORDER BY( time, agent_id, app_name, tenant_app_key, user_id ) TTL createDate + toIntervalDay ( 3 );


CREATE TABLE stresstest_shard_m.t_app_base_data_all (
  `time` Int64,
  `agent_id` String,
  `app_ip` String,
  `app_name` String,
...
CREATE TABLE stresstest_shard_m.t_performance_base_data (
  ...
  `createDate` DateTime DEFAULT toDateTime (
  now()) 
) ENGINE = ReplicatedMergeTree ( '/clickhouse/tables/aiops/t_performance_base_data/{m_shard}', '{replica}' ) PARTITION BY modulo(time,3600000) PRIMARY KEY ( app_name,time )
ORDER BY( app_name,time ) TTL createDate + toIntervalDay ( 7 );


CREATE TABLE stresstest_shard_m.t_performance_base_data_all (
  `time` Int64,
  `timestamp` Int64,
  `total_memory` Int64,
  `perm_memory` Int64,
...
CREATE TABLE stresstest_shard_m.trace_metrics (
  ...
  `createDate` DateTime DEFAULT toDateTime (
  now()) 
) ENGINE = ReplicatedMergeTree ( '/clickhouse/tables/aiops/trace_metrics/{m_shard}', '{replica}' ) PARTITION BY modulo(time,3600000) PRIMARY KEY (time, appName )
ORDER BY( time, appName ) TTL createDate + toIntervalDay ( 3 );

CREATE TABLE stresstest_shard_m.trace_metrics_all (
  `time` Int64,
  `edgeId` String,
  `clusterTest` String,
  `service` String,
...
CREATE TABLE stresstest_shard_m.trace_e2e_assert_metrics_all (
  `time` Int64,
  `nodeId` String,
  `exceptionType` String,
  `traceId` String,
