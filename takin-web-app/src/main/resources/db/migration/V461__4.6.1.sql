...
DECLARE count INT;
SET count = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE table_schema = DATABASE() AND TABLE_NAME = 't_script_manage_deploy' AND COLUMN_NAME = 'create_user_id');
...
DECLARE count INT;
SET count = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE table_schema = DATABASE() AND TABLE_NAME = 't_base_config' AND COLUMN_NAME = 'CONFIG_VALUE');
IF count > 0 THEN
