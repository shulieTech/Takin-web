...
CREATE TABLE aiops_shard_m.t_app_base_data (
  ...
  `createDate` DateTime DEFAULT toDateTime (
  now()) 
) ENGINE = ReplicatedMergeTree ( '/clickhouse/tables/cluster-aiops/t_app_base_data/{m_shard}', '{replica}' ) PARTITION BY app_name PRIMARY KEY ( agent_id, app_name, tenant_app_key, user_id ) 
...
CREATE TABLE aiops_shard_m.t_performance_base_data (
  ...
  `createDate` DateTime DEFAULT toDateTime (
  now()) 
) ENGINE = ReplicatedMergeTree ( '/clickhouse/tables/cluster-aiops/t_performance_base_data/{m_shard}', '{replica}' ) PARTITION BY app_name PRIMARY KEY ( app_name ) 
...
CREATE TABLE aiops_shard_m.trace_metrics (
  ...
  `createDate` DateTime DEFAULT toDateTime (
  now()) 
) ENGINE = ReplicatedMergeTree ( '/clickhouse/tables/cluster-aiops/trace_metrics/{m_shard}', '{replica}' ) PARTITION BY traceId PRIMARY KEY ( traceId, time, appName ) 
