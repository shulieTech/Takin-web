...
public class AppRemoteCallServiceImpl implements AppRemoteCallService {
    ...
    @Resource
    private AppRemoteCallMapper appRemoteCallMapper;
    ...
    private PagingList<AppRemoteCallListVO> getAmdbPagingList(AppRemoteCallQueryInput input, ApplicationDetailResult detailResult,
        ...
        List<AppRemoteCallListVO> outputs = calls.getList().stream()
                ...
                .map(call -> {
                    ...
                    listVO.setMd5(RemoteCallUtils.buildRemoteCallName(listVO.getAppName(), listVO.getInterfaceName(), listVO.getInterfaceType()));
                    //根据md5值去数据库查询数据
                    LambdaQueryWrapper<AppRemoteCallEntity> lambdaQueryWrapper = new LambdaQueryWrapper<>();
                    lambdaQueryWrapper.select(AppRemoteCallEntity::getId);
                    lambdaQueryWrapper.eq(AppRemoteCallEntity::getMd5, listVO.getMd5());
                    AppRemoteCallEntity appRemoteCallEntity = appRemoteCallMapper.selectOne(lambdaQueryWrapper);
                    if (appRemoteCallEntity != null) {
                        listVO.setId(appRemoteCallEntity.getId());
                    }else {
                        listVO.setId(0L);
                    }
                    // 权限问题
